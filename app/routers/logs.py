...
@router.get("/otp", response_model=AuthLogListResponse)
def list_otp_logs(
    ...
    query = (
        db.query(AuthLog)
        .filter(
            AuthLog.actor_id == user_id,
            AuthLog.actor_type == AuthActorType.CLIENT,
            AuthLog.action.in_([AuthAction.OTP_REQUEST, AuthAction.OTP_VERIFICATION]),
        )
